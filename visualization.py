...
def plot_trajectories(ax,
    ...
    for node in histories_dict:
        ...
        for sample_num in range(prediction_dict[node].shape[1]):
            ...
            ax.plot(predictions[batch_num, sample_num, :pred_num, 0], predictions[batch_num, sample_num, :pred_num, 1],
...
def visualize_prediction(ax,
    ...
    plot_trajectories(ax, prediction_dict, pred_num, histories_dict, futures_dict, *kwargs)
...
def visualize_distribution(ax,
    ...
    for node, pred_dist in prediction_distribution_dict.items():
        # print(pred_dist.mus[:, 0:1, :, :, :].shape[:2])
        ...
        # print(covs)
        for timestep in range(pred_num): # (means.shape[0]):
            for z_val in range(means.shape[1]):
                ...
                ell = patches.Ellipse(mean, v[0], v[1], 180. + angle, color='blue' if node.type.name == 'VEHICLE' else 'red')
