...
#import visualization_Mik as vis_mik
import pandas as pd
...
def main():
    ...
    # 'models_03_Sep_2024_17_15_24_int_ee' for dynamic integration
    ...
    for timestep in range(init_timestep + 1, eval_scene.timesteps):
        ...
        dists, preds = trajectron.incremental_forward(input_dict,
                                                      ...
                                                      full_dist=True)
        # Here dists holds covariances, check why it doesnt draw them...
    
        end = time.time()
        print("t=%d: took %.2f s (= %.2f Hz) w/ %d nodes and %d edges" % (timestep, end - start,
                                                                          1. / (end - start), len(trajectron.nodes),
                                                                          trajectron.scene_graph.get_num_edges()))
        # ############################ - Code to change prediction local frame into global frame
        # df_preds = df[(df['Frame_id'] >= timestep+1) & (df['Frame_id'] <= timestep+6)] # prediction 1-6 camera data

        # pred_data = list(preds.values())
        # keys_list = list(preds.keys())

        # preds_dict = {}
        # for i in range(len(pred_data)):
        #     data_row = []
        #     for j in range(len(np.squeeze(pred_data[i]))):
        #         yaw = df_preds.iloc[j].yaw
        #         rotation = np.array([[np.cos(yaw), -np.sin(yaw)], [np.sin(yaw), np.cos(yaw)]])
        #         translation = np.array([df_preds.iloc[j].x, df_preds.iloc[j].y])
        #         final = rotation @ np.squeeze(pred_data[i])[j] + translation
        #         data_row.append(final.tolist())
        #     array_2d = np.array(data_row, dtype=np.float32)
        #     array_4d = array_2d.reshape(1, 1, 6, 2)
        #     preds_dict[keys_list[i]] = array_4d
        
        # preds = preds_dict
        # ############################

        ############################ - Code to change prediction local frame into global frame
        # df_preds = df[df['Frame_id'] == timestep] # prediction at timestep camera data
        ...
        # for i in range(len(pred_data)):
        #     data_row = []
        #     for j in range(len(np.squeeze(pred_data[i])[0])):
        #         yaw = df_preds.yaw.iloc[0]
        #         rotation = np.array([[np.cos(yaw), -np.sin(yaw)], [np.sin(yaw), np.cos(yaw)]])
        #         translation = np.array([df_preds.x.iloc[0], df_preds.y.iloc[0]])
        #         final = rotation @ np.squeeze(pred_data[i])[0][j] + translation
        #         data_row.append(final.tolist())
        #     array_2d = np.array([data_row,np.squeeze(pred_data[i])[1]] , dtype=np.float32)
        #     array_4d = array_2d.reshape(1, 2, 6, 2)
        #     preds_dict[keys_list[i]] = array_4d
        # preds = preds_dict
        ############################
